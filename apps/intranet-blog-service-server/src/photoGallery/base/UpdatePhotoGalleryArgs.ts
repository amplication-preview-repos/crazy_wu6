/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PhotoGalleryWhereUniqueInput } from "./PhotoGalleryWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PhotoGalleryUpdateInput } from "./PhotoGalleryUpdateInput";

@ArgsType()
class UpdatePhotoGalleryArgs {
  @ApiProperty({
    required: true,
    type: () => PhotoGalleryWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PhotoGalleryWhereUniqueInput)
  @Field(() => PhotoGalleryWhereUniqueInput, { nullable: false })
  where!: PhotoGalleryWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PhotoGalleryUpdateInput,
  })
  @ValidateNested()
  @Type(() => PhotoGalleryUpdateInput)
  @Field(() => PhotoGalleryUpdateInput, { nullable: false })
  data!: PhotoGalleryUpdateInput;
}

export { UpdatePhotoGalleryArgs as UpdatePhotoGalleryArgs };
